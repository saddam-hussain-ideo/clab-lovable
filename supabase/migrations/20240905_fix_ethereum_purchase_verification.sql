...
-- Fix the handle_purchase_verification function to properly handle Ethereum transactions
...
BEGIN
  ...
  -- Get current ETH price from settings
  IF v_is_ethereum AND p_currency = 'ETH' THEN
    ...
    IF v_eth_usd_price IS NULL OR v_eth_usd_price <= 0 THEN
      v_eth_usd_price := 3000; -- Default price if not set
      RAISE LOG 'Using default ETH price of $3000 USD';
    ELSE
...
-- Also make sure settings are available for Ethereum
DO $$
BEGIN
  -- Update presale_settings if sol_price column exists but price doesn't
  IF EXISTS (
    SELECT 1 FROM pg_attribute 
    WHERE attrelid = 'presale_settings'::regclass 
    AND attname = 'sol_price'
  ) AND NOT EXISTS (
    SELECT 1 FROM pg_attribute 
    WHERE attrelid = 'presale_settings'::regclass 
    AND attname = 'price'
  ) THEN
    ALTER TABLE presale_settings ADD COLUMN price NUMERIC;
    UPDATE presale_settings SET price = sol_price WHERE price IS NULL;
  END IF;
  
  -- Set a default ETH price in the settings table if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM pg_attribute 
    WHERE attrelid = 'presale_settings'::regclass 
    AND attname = 'eth_price'
  ) THEN
    ALTER TABLE presale_settings ADD COLUMN eth_price NUMERIC DEFAULT 3000;
    
    -- Initialize with real ETH price if possible
    UPDATE presale_settings SET eth_price = 3000 WHERE eth_price IS NULL;
    RAISE NOTICE 'Added eth_price column with default value of 3000';
  END IF;
END $$;
