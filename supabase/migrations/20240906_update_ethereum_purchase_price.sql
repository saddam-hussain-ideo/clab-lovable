...
-- Update the handle_purchase_verification function to accept and use the actual ETH price
...
-- Update the presale_settings schema to ensure eth_price is set with realistic default value
DO $$
BEGIN
  -- Set a default ETH price in the settings table if it's not set
  IF EXISTS (
    SELECT 1 FROM pg_attribute 
    WHERE attrelid = 'presale_settings'::regclass 
    AND attname = 'eth_price'
  ) THEN
    -- Update with more realistic value
    UPDATE presale_settings SET eth_price = 1997 WHERE eth_price = 3000 OR eth_price IS NULL;
    RAISE NOTICE 'Updated eth_price column with default value of 1997';
  END IF;
END $$;
