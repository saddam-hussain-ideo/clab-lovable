...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing pending contributions for network: ${network}, wallet: ${walletAddress || 'all'}`)

    // Build the query for pending transactions
    let query = supabase
    ...
    if (walletAddress) {
      console.log(`Filtering for specific wallet: ${walletAddress}`)
      query = query.ilike('wallet_address', walletAddress)
    }
      
    const { data: pendingContributions, error: fetchError } = await query
    ...
    for (const contribution of pendingContributions) {
      try {
        ...
        console.log(`Processing contribution ${contribution.id} - Status: ${contribution.status}, Network: ${contribution.network}, Verified: ${isVerified}`)
