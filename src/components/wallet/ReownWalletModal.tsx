...
import { ReownWalletOptions } from "./ReownWalletOptions";
...
export const ReownWalletModal = ({ 
  ...
  useEffect(() => {
    if (open) {
      console.log("[ReownWalletModal] Modal opened with network:", network);
      console.log("[ReownWalletModal] Force prompt:", forcePrompt);
      console.log("[ReownWalletModal] Is mobile device:", isMobile);
    }
  ...
  const handleWalletConnection = (connected: boolean, address: string | null, type?: WalletType | null) => {
    try {
      ...
      if (connected && (!address || !type)) {
        console.warn("[ReownWalletModal] Invalid connection data:", { connected, address, type });
        return;
      }
      ...
      console.log("[ReownWalletModal] Connection result:", { 
        connected, 
        address, 
        type,
      ...
      if (connected && address && typeof address === 'string') {
        try {
          ...
          console.error('[ReownWalletModal] Error storing wallet data:', storageError);
        }
      }
      ...
      if (onConnect) {
        try {
          ...
          console.error('[ReownWalletModal] Error in onConnect callback:', callbackError);
        }
      }
      
      // Close the modal only on successful connection
      if (connected && address) {
        console.log("[ReownWalletModal] Closing modal after successful connection");
        onOpenChange(false);
      } else if (!connected) {
        // If connection failed, keep modal open but log it
        console.log("[ReownWalletModal] Connection failed, keeping modal open");
      }
      
    } catch (error) {
      console.error('[ReownWalletModal] Error in handleWalletConnection:', error);
